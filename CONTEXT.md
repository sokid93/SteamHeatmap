...
- **Open**: #10 blended-region visual treatment + color-scale normalization (grill-me started 2026-07-15, paused at the color-scale question — post-threshold tops legitimately span ×1.70–×12.16 and saturate the linear [0,2] domain; log scale was the standing recommendation); #11 map interaction — click to pin a region, hover top-3 popup — lands inside `map.js` behind the existing interface. Architecture-review candidates deliberately deferred: deepen the daily-job composition (`run_daily_job` behind one seam), name-based column reads in `PostgresRankingRepository`.
