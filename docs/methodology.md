...
| 2026-07-31 | An ADR is amendable by implementation evidence: when the first close look at the real artifact shows a decision was scoped on a wrong premise, the ADR is amended and its issues re-scoped *before* any code, with the amendment as its own commit | ADR-015 scoped the visual pass as restyle-only ("existing regions are restyled in place"); looking at the live page before starting showed the map's lack of weight was structural — the only unframed element, fifth in a stack of full-width blocks, a third of its row given to a ten-item list — so a new palette would only have repainted the same hierarchy |
| 2026-07-31 | A refactor claimed to change nothing visible proves it: render before and after, compare the bytes | The `site.css` token extraction (`0eb96d8`) rewrote every colour and spacing value in the file. An identical screenshot hash is what made "no value drifted" a fact instead of a hope, and left the palette change reviewable on its own |
| 2026-08-08 | An external API endpoint a grill-me design leans on gets a live call before the issue is scoped, not just recalled from memory — ADR-012 already modeled this ("Findings (verified, not assumed)") but ADR-016 didn't repeat it | Issue #24 (weekly catalog cache) was designed and fully implemented around `ISteamApps/GetAppList`, assumed keyless like every other endpoint this project uses; only discovered at live-run verification that Valve removed it, and its replacement is key-gated — a session's worth of correct code paused on a premise nobody had actually checked |
| 2026-07-31 | Design decisions that have a measurable criterion — contrast ratios, sequential ramps — are settled by running the check and recording the numbers, never by eye | The first dark heatmap ramp looked fine and failed its low-end contrast at 1.30:1. The AA claim for the new palette is now a table of ratios measured from rendered colours, and the two separators that sit below 3:1 are reported as deliberate rather than quietly passed over |
| 2026-07-31 | Where the work deviates from an issue's stated acceptance criteria, the deviation and its reason are written onto the issue — never silently absorbed on the grounds that the result is better | Four times in one pass: Manrope ships as one variable file rather than the two the ADR assumed, Fiji is excluded from the map crop, the type scale has no eyebrow role, and the hover transition leaves out `fill` |
| 2026-07-31 | With deployment on push, "push once per completed feature" means once per *coherent user-visible state*, not per closed issue — commits that would ship a visibly broken page are held until the issue that resolves them lands | Finishing #16 deliberately left the map's fills inverted, the light no-data grey being the brightest thing on a dark page, with #17 owning the fix; `deploy-web.yml` ships any push touching `web/`, so pushing that finished sub-issue on its own would have put it on the live site |
| 2026-08-14 | When logic is deliberately duplicated across the C#/Python boundary (ADR-006 keeps the two runtimes independent — no shared library), the port is tested against the *other side's exact golden values*, never independently re-derived. A silent mismatch here is worse than in a same-language change: it wouldn't fail loudly, it would just paint an on-demand-fetched game a different color than one the daily pipeline scored | Issue #27's `OnDemandScoring` (C#) had to match `analysis/steamheatmap/scoring.py`'s Wilson/concentration math bit-for-bit — a game searched live and one picked up by tomorrow's batch run must land on the same number. The C# tests reuse `test_scoring.py`'s exact expected values (0.9630, 1.5, etc.) rather than recomputing them |
